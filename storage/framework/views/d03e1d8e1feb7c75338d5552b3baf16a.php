                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-3 mb-2">
                                    ...
                                    <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium 
                                        ...
                                        <?php endif; ?>">
                                        <?php echo e(ucfirst($child->order_type)); ?> Items
                                    </span>
                                    <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium <?php echo e($childStatusColor); ?>">
                                        <?php echo e(ucwords(str_replace('_', ' ', $child->status))); ?>

                                    </span>
                                    <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium <?php echo e($childPaymentColor); ?>">
                                        <?php echo e(ucwords(str_replace('_', ' ', $childPaymentStatus))); ?>

                                    </span>
                                </div>
                                <div class="text-xs text-gray-600">
                                    Payment Method: <span class="font-medium"><?php echo e($child->payment_method ?? 'N/A'); ?></span>
                                    <?php if($child->remaining_balance > 0): ?>
                                        | Remaining: <span class="font-medium text-blue-700">₱<?php echo e(number_format($child->remaining_balance, 2)); ?></span>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 ml-4">
                                <span class="text-sm font-medium text-gray-700">₱<?php echo e(number_format($child->total_amount, 2)); ?></span>
                                <a href="<?php echo e(route('employee.orders.show', $child->id)); ?>" class="inline-flex items-center px-3 py-1.5 rounded-lg border border-purple-300 text-purple-700 hover:bg-purple-100 text-xs font-medium">Manage Sub-Order</a>
                            </div>
                        </div>
                ...
                <select name="status" class="rounded-md border border-gray-300 text-sm px-3 py-2">
                    ...
                    <?php $__currentLoopData = $availableStatuses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <option value="<?php echo e($s); ?>" <?php if($isCurrentStatus): echo 'selected'; endif; ?> <?php if($disabled): echo 'disabled'; endif; ?>>
                            ...
                            <?php if($disabled): ?>
                                <?php if($order->requiresFinalPaymentVerification()): ?>
...
unset($__errorArgs, $__bag); ?>
                ...
                <?php if(in_array($order->status, ['ready_to_ship', 'shipped', 'delivered', 'completed', 'ready_for_delivery'])): ?>
                    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                                    ...
                                    <div class="flex items-center justify-between bg-white px-3 py-2 rounded border border-blue-100">
                                        <div class="flex-1">
                                            <div class="flex items-center gap-2 mb-1">
                                                <span class="text-sm font-medium text-gray-900">Sub-Order #<?php echo e($child->id); ?></span>
                                                <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium 
                                                    <?php if($child->order_type === 'standard'): ?> bg-green-100 text-green-800
                                                    <?php else: ?> bg-blue-100 text-blue-800
                                                    <?php endif; ?>">
                                                    <?php echo e(ucfirst($child->order_type)); ?>

                                                </span>
                                                <span class="inline-flex px-2 py-0.5 rounded text-xs font-medium 
                                                    <?php if($childPaymentStatus === 'paid'): ?> bg-green-100 text-green-800
                                                    <?php elseif($childPaymentStatus === 'partially_paid'): ?> bg-blue-100 text-blue-800
                                                    <?php elseif($childPaymentStatus === 'pending_verification' || $childHasPendingVerification): ?> bg-yellow-100 text-yellow-800
                                                    <?php elseif($childPaymentStatus === 'pending_cod'): ?> bg-blue-100 text-blue-800
                                                    <?php else: ?> bg-red-100 text-red-800
                                                    <?php endif; ?>">
                                                    <?php echo e(ucwords(str_replace('_', ' ', $childPaymentStatus))); ?>
                                        ...
                                        <a href="<?php echo e(route('employee.orders.show', $child->id)); ?>" class="ml-4 inline-flex items-center px-3 py-1.5 rounded-lg border border-blue-300 text-blue-700 hover:bg-blue-100 text-xs font-medium">Verify Payment →</a>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        </div>
                            ...
                            <div class="border-t border-gray-200 pt-4 mt-4">
                                ...
                                <?php if($finalPaymentVerified): ?>
                                    <div class="bg-green-50 border border-green-200 rounded-lg p-3">
                                        ...
                                        <?php endif; ?>
                                        <?php if($order->final_payment_verification_notes): ?>
                                            <p class="text-xs text-green-700 mt-2">
                                                <strong>Notes:</strong> <?php echo e($order->final_payment_verification_notes); ?>

                                            </p>
                                        <?php endif; ?>
                                    </div>
                                <?php elseif($requiresFinalVerification): ?>
                                    ...
                                    <form method="POST" action="<?php echo e(route('employee.orders.verify-final-payment', $order->id)); ?>" id="verifyFinalPaymentForm" class="space-y-4">
                                        <?php echo csrf_field(); ?>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Verification Action:</label>
                                            <div class="space-y-2">
