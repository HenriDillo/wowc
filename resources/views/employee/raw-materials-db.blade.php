...
@section('content')
    ...
    <form action="{{ route('employee.materials.store') }}" method="POST" class="bg-white p-4 rounded-md border border-gray-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        @csrf
        <input type="text" name="name" placeholder="Material name" class="border rounded px-3 py-2" required>
        ...
        <button class="bg-[#c49b6e] text-white px-4 py-2 rounded hover:bg-[#b08a5c]">Add Material</button>
    </form>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-4 py-3 text-sm">
                        <form action="{{ route('employee.materials.update', $m) }}" method="POST" class="inline-flex items-center space-x-2">
                            @csrf
                            @method('PUT')
                            <input type="text" name="name" value="{{ $m->name }}" class="border rounded px-2 py-1 w-40">
                            <input type="number" name="stock" value="{{ $m->stock }}" class="border rounded px-2 py-1 w-20" min="0">
                            <input type="number" step="0.01" name="price" value="{{ $m->price }}" class="border rounded px-2 py-1 w-24" min="0">
                            <button class="text-blue-600 hover:underline">Save</button>
                        </form>
                        <form action="{{ route('employee.materials.toggle', $m) }}" method="POST" class="inline">
                            @csrf
                            @method('PATCH')
                            <button class="ml-3 text-gray-700 hover:underline">{{ $m->visible ? 'Hide' : 'Unhide' }}</button>
                        </form>
                    </td>
                </tr>
                @endforeach
